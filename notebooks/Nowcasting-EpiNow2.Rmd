...
Plotting weekly incidence of cases makes clear the right-truncation bias present in the snapshots of the data:

```{r legacy data plot, echo=FALSE}
...
Another important time interval that will be useful to correct this right truncation bias is known as the **generation time**, that corresponds to the time elapsed between the infection of an infector and an infectee. A schematic of these delays can be seen in the following figure:

![Delay distributions scheme](./images/delays_fig.png)
...
p_trunc[[3]] <- p_trunc[[3]] +
  labs(y = "Effective reproduction no. \n by date of infection")
#   geom_line(
